...
class VisualizationJob(Job):
    ...
    Args:
        ...
        add_images_subdir (bool): Whether to add 'images' subdirectory. Defaults to True.
    """
    ...
    def __init__(
        ...
        root_dir: Path, 
        data_args: dict,
        add_images_subdir: bool = True,
    ) -> None:
        super().__init__()
        self.predictions = predictions
        # Optionally add 'images' subdirectory
        self.root_dir = root_dir / "images" if add_images_subdir else root_dir
...
class VisualizationJobGenerator(JobGenerator):
    ...
    Args:
        root_dir (Path): Root directory where images will be saved.
        data_args (dict): Data configuration arguments.
        add_images_subdir (bool): Whether to add 'images' subdirectory. Defaults to True.
    """

    def __init__(self, root_dir: Path, data_args: dict, add_images_subdir: bool = True) -> None:
        self.root_dir = root_dir
        self.data_args = data_args
        self.add_images_subdir = add_images_subdir
    ...
    def generate_jobs(
        ...
        if prev_stage_result is not None:
            yield VisualizationJob(
                prev_stage_result, 
                self.root_dir, 
                data_args=self.data_args,
                add_images_subdir=self.add_images_subdir,
            )
        else:
