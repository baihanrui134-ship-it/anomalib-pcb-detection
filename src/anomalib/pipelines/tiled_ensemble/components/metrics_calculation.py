...
class MetricsCalculationJob(Job):
    ...
    def run(self, task_id: int | None = None) -> dict:
        ...
        for metric in self.evaluator.test_metrics:
            ...
            # Some metrics may return None if not enough data or computation failed
            if result is not None:
                metrics_dict[metric.name] = result.item()
            else:
                logger.warning(f"Metric {metric.name} returned None, skipping.")
            metric.cpu()
