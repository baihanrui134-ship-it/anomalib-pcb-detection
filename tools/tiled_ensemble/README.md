...
- **PatchCore（本项目使用）**：建立"正常特征库"，新样本与库中特征对比，差异大则为异常。无需梯度训练，精度最高（99.6%）。
...
**结论**：虽然 PatchCore 不是"最快"的，但 **1-2 秒的速度满足工业产线需求**。
...
- 各模型依次工作，分别学习各自区域的正常特征
- 预测时将各 tile 的异常热力图拼接成完整结果

**实际效果**：
- 检测精度无明显变化（相比单模型处理整图）
- 显存占用大幅降低（每个 tile 独立处理）
...
**功能价值**：自动去除背景区域，解决实际生产中 **PCB 高度不一致导致的误判问题**

**实现原理**：
- 基于 Otsu 二值化算法自动检测 PCB 边缘
- 裁剪掉图像四周的背景，只保留 PCB 有效区域
- 支持边距调整（padding）和最小面积过滤（min_area_ratio）

**实际效果（解决的痛点）**：

在实际生产环境中，PCB 板的拍摄高度常常不一致，导致以下问题：
- **问题 1**：高度不同 → 背景区域大小不同 → 模型将背景变化误判为异常
- **问题 2**：背景占据大量像素 → 干扰异常分数计算 → 误报率高
...
**图片说明：**
- **左图（Original）**：原始图像，包含大量背景区域
- **中图（Detection）**：检测到的 PCB 边界（绿色框）
- **右图（Cropped - No BG）**：裁剪后的纯净 PCB 图像，背景大量去除
...
⏱️  整个训练过程通常只需 5-15 分钟（取决于数据量与显卡）
```
